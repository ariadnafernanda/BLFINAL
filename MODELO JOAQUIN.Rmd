...
escuela_tobit <- VGAM::vglm(WPI ~ ., tobit(Upper = 1), data = LatenLigas[,-c(1)])
...
```{r}
fviz_nbclust(LatenLigas[,c(2:8)], pam,diss=g.dist,method = "gap_stat",k.max = 10,verbose = F)
```

```{r}
res.pam = pam(g.dist,3,cluster.only = F)
LatenLigas$clusterPAM=res.pam$cluster
```
...
```{r}
LatenLigas$clusterDIANA=res.diana$cluster
```

```{r}
LatenLigas[LatenLigas$clusterDIANA==1,'Country']
```

```{r}
LatenLigas[LatenLigas$clusterDIANA==2,'Country']
```

```{r}
LatenLigas[LatenLigas$clusterDIANA==3,'Country']
```

```{r}
LatenLigas[LatenLigas$clusterPAM==1,'Country']
```

```{r}
LatenLigas[LatenLigas$clusterPAM==2,'Country']
```

```{r}
LatenLigas[LatenLigas$clusterPAM==3,'Country']
```
