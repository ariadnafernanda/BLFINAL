...
```
#Una vez que tenemos la data final, empezamos con la comparacion de Clusters"
```{r}
...
LatenLigas$clusterPT=res.pam$cluster
LatenLigas$clustAG=res.agnes$cluster
LatenLigas$clustDIV=res.diana$cluster
```

```{r}
#Visualizacion PARTICIONANTE
fviz_cluster(object = list(data=g.dist, cluster = LatenLigas$clusterPT),
             geom = c("text"), 
             ellipse.type = "convex")
```

```{r}
#Visualizacion AGLOMERATIVA
fviz_dend(res.agnes,k=3, cex = 0.3, horiz = T)
```
...
aggDIV=aggregate(as.matrix(cbind(LatenLigas[,c(2:8)]))~ clustDIV, data=LatenLigas,
...
```
```{r}
#Para una mejor presentacion en la Web
library(knitr)
library(kableExtra)
kable(tablaResumenDIV, format = "html", digits = 2)%>%
  kable_styling()
```
