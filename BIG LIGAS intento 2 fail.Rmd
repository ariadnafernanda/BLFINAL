...
comp_rep <- function(x,y) {
  read.xlsx('https://github.com/sergioromero1917/Data-Estad-sitica-2/raw/master/PDF%20competivines%20report.xlsx',sheet = x, rows = y,colNames = T, rowNames = F)}
data1=rbind(comp_rep(2,f1),comp_rep(3,f))
data2=rbind(comp_rep(4,f),comp_rep(5,f))
data3=rbind(comp_rep(6,f),comp_rep(7,f))

MyMerge1 <- function(x, y){
   df <- merge(x, y, by= "Economy", all.x= TRUE, all.y= TRUE)
  return(df)} 

compindex=Reduce(MyMerge1, list(data1,data2,data3))
```
...
modelo <- ' ambiente_apto  =~ Institutions + Infrastructure + ICT_adoption + Macroeconomic_stability
           ...
           mercados   =~ Product_market + Labour_market + Financial_system + Market_size
           innovacion_tecno =~ Business_dynamism + Innovation_capability'
```

```{r}
final_ligas=dataBigLigas[,-c(25,26)]
names(final_ligas)
...
```{r}
library(VGAM)
LatenLigas$WPI=LatenLigas$WPI/100
...
```{r}
library(cluster)
fviz_nbclust(LatenLigas[,-c(1,9,3)], pam,diss=g.dist,method = "gap_stat",k.max = 10,verbose = F)
```
